...
2. The app invokes Android's native "open with" flow using the file's stored local path and MIME type, via `expo-sharing`'s `shareAsync` (which on Android surfaces the system share/open sheet) — this is the Expo API chosen for this flow, since it is designed to hand a locally-stored file to another installed app without requiring a custom `Intent`/`expo-intent-launcher` integration. **(assumption/architectural pick: `expo-sharing`'s `shareAsync` is used both for viewing attachments and for the export share sheet — see below — keeping the app to a single well-supported Expo module for this class of interaction rather than mixing in `expo-intent-launcher` or raw `Linking` intents.)**
3. Whatever apps the user already has installed that can handle the file's MIME type are offered by the OS. If none are installed, the OS's own "no app found" handling applies — UniTask does not attempt to catch or work around this.
...
- **Export**: all local data is serialized to a single JSON file, written to app-private storage via `expo-file-system`, then handed to the OS via `expo-sharing`'s `shareAsync` so the user can save/send it anywhere (the same Expo API used for attachment viewing above).
