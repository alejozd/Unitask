...
- Tasks 1-5: **complete and pushed** to `origin/master` (HEAD `1b30246`). Task-by-task:
...
- Tasks 6-7: **pending**, awaiting explicit user OK before Task 6 starts.
  - Task 6: wire `AttachmentList` into `app/tarea/[id]/index.tsx` (Task detail screen).
  - Task 7: full Phase 5 DoD verification (combined check + one short on-device pass), then the final whole-branch review + fix round (if needed) + push, matching every prior phase's close-out pattern.
- Full combined check as of Task 5: 134/134 tests (15 suites), `tsc`/`lint`/`prettier` all clean.
