...
jest.mock("../client", () => ({ authenticatedFetch: jest.fn(), getAccessToken: jest.fn().mockReturnValue("at-1") }));
...
describe("pullChanges", () => {
  ...
  it("applies an upsert and advances the persisted cursor", async () => {
    ...
    (authenticatedFetch as jest.Mock).mockResolvedValueOnce(jsonResponse({ operations: [], cursor: 5 }));
  ...
  it("applies a delete", async () => {
    ...
    (authenticatedFetch as jest.Mock).mockResolvedValueOnce(jsonResponse({ operations: [], cursor: 1 }));
  ...
  it("reschedules the OS notification for a pulled reminder with a future fire time", async () => {
    ...
    await db.insert(subjects).values({ id: "subj-1", name: "Física", courseCode: null, professorName: null, color: "indigo", semesterId: "sem-3", createdAt: new Date(), updatedAt: new Date() });
  ...
  it("cancels the old OS notification before rescheduling an already-pulled reminder", async () => {
    ...
    await db.insert(subjects).values({ id: "subj-2", name: "Química", courseCode: null, professorName: null, color: "indigo", semesterId: "sem-4", createdAt: new Date(), updatedAt: new Date() });
  ...
  it("downloads a pulled attachment's file when the local file doesn't exist yet", async () => {
    ...
    await db.insert(semesters).values({ id: "sem-5", label: "s", status: "active", createdAt: new Date() });
    await db.insert(subjects).values({ id: "subj-3", name: "Arte", courseCode: null, professorName: null, color: "indigo", semesterId: "sem-5", createdAt: new Date(), updatedAt: new Date() });
    await db.insert(tasks).values({ id: "task-3", title: "T", description: null, subjectId: "subj-3", dueDateTime: new Date(), priority: "Baja", completed: false, completedAt: null, completedLate: false, createdAt: new Date(), updatedAt: new Date() });
    (authenticatedFetch as jest.Mock).mockResolvedValueOnce(
      jsonResponse({
        operations: [
          {
